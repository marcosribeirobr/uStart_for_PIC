...
namespace BootProgUsb
{
	public partial class MainForm : Form
	{
      	...
      	private bool Send( Comandos Cmd, ulong Address, int offset, byte[] Dados )
		{
			...
			{
				MessageBox.Show( "Falha ao enviar os dados" );
				return false;
			}
      		...
      		if( e.data[1] == STX )
      		{
      			switch( e.data[CMD] )
      			{
      				case (byte)Comandos.INFO:
      					...
      					MainForm_Disconnect.Invoke( (MethodInvoker) delegate { MainForm_Disconnect.Text = "DESCONECTAR"; } ); //RETIREI DESCONECTAR
		...
		void MainForm_Open_FileClick(object sender, EventArgs e)
		{
			if( openFile.ShowDialog() == DialogResult.OK )
			{
				...
				if( ArquivoHexadecimal.Read( openFile.FileName, ref Flash, ref Eeprom ) )
				{
					//Atualiza o status
					this.ProgramStatus = "Arquivo Carregado: " + ArquivoHexadecimal.Bytes.ToString() + " Bytes";
		...
		{
			string msg =   "FLASH: \t\t\t" + bootInfo.FlashSize + " Bytes\r" + 
						"EEPROM: \t\t\t" + bootInfo.EepromSize + " Bytes\r" +
						"BOOTLOADER ADDRESS: \t" + bootInfo.BootStart + "\n" + 
						"ERASE BLOCKS: \t\t" + bootInfo.EraseBlock + "\n\n" + "      Contato: marcosribeiro.br@gmail.com" ;
        ...
        private void MainForm_USB_Status_Click(object sender, EventArgs e)
        {

        }

        private void MainForm_Progress_Click(object sender, EventArgs e)
        {

        }

        private void MainForm_MCU_Device_Click(object sender, EventArgs e)
        {
        ...
        private void MainForm_Status_Click(object sender, EventArgs e)
        {
